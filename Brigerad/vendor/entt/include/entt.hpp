...
struct std_sort
{
    ...
    {
        std::sort(
          std::forward<Args>(args)..., std::move(first), std::move(last), std::move(compare));
    }
};
    ...
    void operator()(It first, It last, Getter getter = Getter {}) const
    {
        if (first < last)
        {
            ...
            auto part = [getter = std::move(getter)](auto from, auto to, auto out, auto start) {
                ...
                std::for_each(std::next(std::begin(index)),
                              std::end(index),
                              [index = std::begin(index), count = std::begin(count)](
                                auto& item) mutable { item = *(index++) + *(count++); });
...
public:
    ...
    // at the time I'm writing, clang crashes during compilation if auto is used instead of
    // family_type
...
public:
     ...
     */
    template<std::size_t N>
    constexpr basic_hashed_string(const value_type (&curr)[N]) ENTT_NOEXCEPT : str {curr},
                                                                               hash {helper(curr)}
    {
    }
 ...
 */
template<typename Char>
constexpr bool operator!=(const basic_hashed_string<Char>& lhs,
...
class identifier
{
    ...
    static constexpr id_type get(std::index_sequence<Indexes...>)
    {
        ...
        return (0 + ... +
                (std::is_same_v<Type, std::tuple_element_t<Indexes, tuple_type>> ? id_type(Indexes)
                                                                                 : id_type {}));
    }
...
struct ENTT_API type_info
{
    ...
    {
        ENTT_PRETTY_FUNCTION_CONSTEXPR(static const)
        auto value = entt::hashed_string::value(ENTT_PRETTY_FUNCTION);
        return value;
    }
#else
...
public:
    ...
    void connect() ENTT_NOEXCEPT
    {
        ...
        {
            fn = wrap<Candidate>(
              internal::index_sequence_for<std::tuple_element_t<0, std::tuple<Args...>>>(
...
class sink<Ret(Args...)>
{
    ...
    using difference_type = typename std::iterator_traits<typename decltype(
      signal_type::calls)::iterator>::difference_type;
...
public:
    ...
    sink before(Type* value_or_instance)
    {
        ...
        if (value_or_instance)
        {
            const auto& calls = signal->calls;
            const auto  it =
              std::find_if(calls.cbegin(), calls.cend(), [value_or_instance](const auto& delegate) {
...
public:
    ...
    {
        return (to_integral(entity) & traits_type<Entity>::entity_mask) ==
...
class sparse_set
{
    ...
    page_type& assure(const std::size_t pos)
    {
        ...
        if (!sparse[pos])
        {
            ...
            for (auto *first = sparse[pos].get(), *last = first + entt_per_page; first != last;
                 ++first)
            {
                *first = null;
            }
...
public:
    ...
    /*! @copydoc contains */
    [[deprecated("use ::contains instead")]] bool has(const entity_type entt) const
    {
    ...
    /*! @copydoc emplace */
    [[deprecated("use ::emplace instead")]] void construct(const entity_type entt)
    {
        emplace(entt);
    }
    ...
    /*! @copydoc insert */
    template<typename It>
    [[deprecated("use ::insert instead")]] void construct(It first, It last)
    {
    ...
    /*! @copydoc erase */
    [[deprecated("use ::erase instead")]] void destroy(const entity_type entt) { erase(entt); }
...
class storage : public sparse_set<Entity>
{
    ...
    class storage_iterator final
    {
        ...
        storage_iterator(instance_type& ref, const index_type idx) ENTT_NOEXCEPT : instances {&ref},
                                                                                   index {idx}
        {
        }
...
public:
    ...
    {
        return underlying_type::contains(entt) ? (instances.data() + underlying_type::index(entt))
                                               : nullptr;
    }
    ...
    /*! @copydoc emplace */
    template<typename... Args>
    [[deprecated("use ::emplace instead")]] void construct(const entity_type entt, Args&&... args)
    {
    ...
    /*! @copydoc insert */
    template<typename It>
    [[deprecated("use ::insert instead")]] std::
      enable_if_t<std::is_same_v<typename std::iterator_traits<It>::value_type, entity_type>, void>
    ...
    template<typename EIt, typename CIt>
    [[deprecated("use ::insert instead")]] std::
      enable_if_t<std::is_same_v<typename std::iterator_traits<EIt>::value_type, entity_type>, void>
    ...
    void sort(iterator first, iterator last, Compare compare, Sort algo = Sort {}, Args&&... args)
    {
        ...
        const auto apply = [this](const auto lhs, const auto rhs) {
            std::swap(instances[underlying_type::index(lhs)],
...
class basic_group<Entity, exclude_t<Exclude...>, get_t<Get...>>
{
    ...
    using pool_type = std::conditional_t<std::is_const_v<Component>,
                                         const storage<Entity, std::remove_const_t<Component>>,
                                         storage<Entity, Component>>;

    // we could use pool_type<Type> &..., but vs complains about it and refuses to compile for
    // unknown reasons (most likely a bug)
    basic_group(sparse_set<Entity>& ref,
                storage<Entity, std::remove_const_t<Get>>&... gpool) ENTT_NOEXCEPT
    : handler {&ref},
      pools {&gpool...}
    {
    }
...
public:
    ...
    {
        using get_type_list =
          type_list_cat_t<std::conditional_t<ENTT_IS_EMPTY(Get), type_list<>, type_list<Get>>...>;
     ...
     */
    template<typename Func>
    [[deprecated("use ::each instead")]] void less(Func func) const
    {
        each(std::move(func));
    }
...
class basic_group<Entity, exclude_t<Exclude...>, get_t<Get...>, Owned...>
{
    ...
    // we could use pool_type<Type> &..., but vs complains about it and refuses to compile for
    // unknown reasons (most likely a bug)
    basic_group(const std::size_t& ref,
    ...
    void traverse(Func func, type_list<Strong...>, type_list<Weak...>) const
    {
        [[maybe_unused]] auto it =
          std::make_tuple((std::get<pool_type<Strong>*>(pools)->end() - *length)...);
        ...
        for (auto next = *length; next; --next)
        {
            ...
            else
            {
                const auto entt = *(data++);
                func(entt,
                     *(std::get<component_iterator<Strong>>(it)++)...,
                     std::get<pool_type<Weak>*>(pools)->get(entt)...);
            }
        }
    }
...
public:
    ...
    void each(Func func) const
    {
        using owned_type_list = type_list_cat_t<
          std::conditional_t<ENTT_IS_EMPTY(Owned), type_list<>, type_list<Owned>>...>;
        using get_type_list =
          type_list_cat_t<std::conditional_t<ENTT_IS_EMPTY(Get), type_list<>, type_list<Get>>...>;
...
class basic_runtime_view
{
    ...
    {
        const auto it =
          std::min_element(pools.begin(), pools.end(), [](const auto* lhs, const auto* rhs) {
...
public:
    ...
    template<typename Archive>
    [[deprecated(
      "use ::entities instead, it exports now also destroyed entities")]] const basic_snapshot&
    destroyed(Archive&) const
    {
        return *this;
    }
...
public:
    ...
    template<typename Archive>
    [[deprecated(
      "use ::entities instead, it imports now also destroyed "
      "entities")]] const basic_snapshot_loader&
    destroyed(Archive&) const
    {
        return *this;
    }
...
class basic_continuous_loader
{
    ...
    void restore(Entity entt)
    {
        ...
        {
            remloc[entt].first =
              reg->valid(remloc[entt].first) ? remloc[entt].first : reg->create();
            // set the dirty flag
    ...
    auto update(int, Container& container) -> decltype(typename Container::mapped_type {}, void())
    {
        ...
        {
            using first_type =
              std::remove_const_t<typename std::decay_t<decltype(pair)>::first_type>;
            using second_type = typename std::decay_t<decltype(pair)>::second_type;

            if constexpr (std::is_same_v<first_type, entity_type> &&
...
public:
    ...
    template<typename Archive>
    [[deprecated(
      "use ::entities instead, it imports now also destroyed entities")]] basic_continuous_loader&
    destroyed(Archive&)
    {
        return *this;
    }
    ...
    /*! @copydoc contains */
    [[deprecated("use ::contains instead")]] bool has(entity_type entt) const ENTT_NOEXCEPT
    {
        return contains(entt);
    }
...
class basic_view<Entity, exclude_t<Exclude...>, Component...>
{
    ...
    using pool_type = std::conditional_t<std::is_const_v<Comp>,
                                         const storage<Entity, std::remove_const_t<Comp>>,
                                         storage<Entity, Comp>>;
    ...
    class view_iterator final
    {
        ...
        view_iterator(const sparse_set<Entity>& candidate,
        ...
        bool valid() const
        {
            return std::all_of(unchecked.cbegin(),
                               unchecked.cend(),
                               [entt = *it](const sparse_set<Entity>* curr) {
                                   return curr->contains(entt);
                               }) &&
                   std::none_of(
                     filter.cbegin(), filter.cend(), [entt = *it](const sparse_set<Entity>* curr) {
                         return curr->contains(entt);
                     });
        }
    ...
    // we could use pool_type<Component> &..., but vs complains about it and refuses to compile for
    // unknown reasons (likely a bug)
    ...
    {
        return *std::min(
          {static_cast<const sparse_set<Entity>*>(std::get<pool_type<Component>*>(pools))...},
    ...
    unchecked_type unchecked(const sparse_set<Entity>& view) const
    {
        std::size_t    pos {};
        unchecked_type other {};
        ((std::get<pool_type<Component>*>(pools) == &view
            ? nullptr
    ...
    decltype(auto) get([[maybe_unused]] component_iterator<Comp> it,
                       [[maybe_unused]] pool_type<Other>*        cpool,
    ...
    void traverse(Func func, type_list<Type...>) const
    {
        if constexpr (std::disjunction_v<std::is_same<Comp, Type>...>)
        {
            ...
            for (const auto entt :
                 static_cast<const sparse_set<entity_type>&>(*std::get<pool_type<Comp>*>(pools)))
            {
                auto curr = it++;

                if (((std::is_same_v<Comp, Component> ||
                      std::get<pool_type<Component>*>(pools)->contains(entt)) &&
                     ...) &&
                    (!std::get<pool_type<Exclude>*>(pools)->contains(entt) && ...))
                {
                    ...
                    {
                        func(entt,
                             get<Comp, Type>(curr, std::get<pool_type<Type>*>(pools), entt)...);
                    }
                }
            }
        }
...
public:
    ...
    {
        const auto& view = candidate();
        ((std::get<pool_type<Component>*>(pools) == &view ? each<Component>(std::move(func))
                                                          : void()),
         ...);
    }
    ...
    void each(Func func) const
    {
        using non_empty_type = type_list_cat_t<
          std::conditional_t<ENTT_IS_EMPTY(Component), type_list<>, type_list<Component>>...>;
...
class basic_registry
{
    ...
    struct pool_handler final : storage<Entity, Component>
    {
        ...
        decltype(auto) replace(basic_registry&            owner,
                               const Entity               entt,
                               [[maybe_unused]] Component component)
        {
            ...
            {
                return patch(
                  owner, entt, [&component](auto&& curr) { curr = std::move(component); });
            }
        }
    ...
    struct group_handler<exclude_t<Exclude...>, get_t<Get...>, Owned...>
    {
        static_assert(std::conjunction_v<std::is_same<Owned, std::decay_t<Owned>>...,
                                         std::is_same<Get, std::decay_t<Get>>...,
        ...
        {
            static_assert(std::disjunction_v<std::is_same<Owned, std::decay_t<Owned>>...,
                                             std::is_same<Get, std::decay_t<Get>>...,
                                             std::is_same<Exclude, std::decay_t<Exclude>>...>);
            [[maybe_unused]] const auto cpools = std::forward_as_tuple(owner.assure<Owned>()...);

            const auto is_valid =
              ((std::is_same_v<Component, Owned> ||
                std::get<pool_handler<Owned>&>(cpools).contains(entt)) &&
               ...) &&
            ...
            else
            {
                if (is_valid && !(std::get<0>(cpools).index(entt) < current))
                {
                    ...
                    (std::get<pool_handler<Owned>&>(cpools).swap(
                       std::get<pool_handler<Owned>&>(cpools).data()[pos], entt),
                     ...);
                }
            }
        }
    ...
    const pool_handler<Component>& assure() const
    {
        ...
        if constexpr (has_type_index_v<Component>)
        {
            ...
            if (auto&& pdata = pools[index]; !pdata.pool)
            {
                ...
                pdata.remove = [](sparse_set<entity_type>& cpool,
        ...
        else
        {
            ...
            if (auto it = std::find_if(pools.begin(),
                                       pools.end(),
                                       [id = type_info<Component>::id()](const auto& pdata) {
    ...
    {
        return const_cast<pool_handler<Component>&>(
...
public:
    ...
    entity_type create()
    {
        ...
        else
        {
            ...
            const auto version = to_integral(entities[curr]) &
    ...
    entity_type create(const entity_type hint)
    {
        ...
        else if (const auto curr = (to_integral(entities[req]) & traits_type::entity_mask);
                 req == curr)
        {
            entt = create();
        }
        else
        {
            ...
            for (; (to_integral(*it) & traits_type::entity_mask) != req;
                ...
                ;
            *it = entity_type {
              curr | (to_integral(*it) & (traits_type::version_mask << traits_type::entity_shift))};
    ...
    {
        ENTT_ASSERT(std::all_of(pools.cbegin(), pools.cend(), [](auto&& pdata) {
            return !pdata.pool || pdata.pool->empty();
        }));
        ...
        for (std::size_t pos {}, end = entities.size(); pos < end; ++pos)
        {
            ...
            {
                const auto version = to_integral(entities[pos]) &
                                     (traits_type::version_mask << traits_type::entity_shift);
    ...
    void destroy(const entity_type entity, const version_type version)
    {
        ...
        entities[entt] =
          entity_type {to_integral(destroyed) | (version << traits_type::entity_shift)};
    ...
    /*! @copydoc emplace */
    template<typename Component, typename... Args>
    [[deprecated("use ::emplace instead")]] decltype(auto) assign(const entity_type entity,
                                                                  Args&&... args)
    {
    ...
    {
        static_assert(
          std::is_constructible_v<Component, typename std::iterator_traits<CIt>::value_type>);
        ENTT_ASSERT(std::all_of(first, last, [this](const auto entity) { return valid(entity); }));
      ...
      assign(EIt first, EIt last, CIt value)
    {
        return insert<Component>(
          std::move(first), std::move(last), value, value + std::distance(first, last));
    }
     ...
     *
     * @code{.cpp}
     * auto &component = registry.has<Component>(entity) ? registry.replace<Component>(entity,
     * args...) : registry.emplace<Component>(entity, args...);
     * @endcode
     *
    ...
    template<typename Component, typename... Args>
    [[deprecated("use ::emplace_or_replace instead")]] decltype(auto) assign_or_replace(
    ...
    /*! @copydoc patch */
    template<typename Component, typename... Func>
    [[deprecated("use registry::patch instead")]] auto replace(const entity_type entity,
                                                               Func&&... func)
    ...
    auto replace(const entity_type entity, Args&&... args)
      -> decltype(std::enable_if_t<sizeof...(Args) != 0>(),
    ...
    decltype(auto) get_or_emplace(const entity_type entity, Args&&... args)
    {
        ENTT_ASSERT(valid(entity));
        auto& cpool = assure<Component>();
        return cpool.contains(entity) ? cpool.get(entity)
                                      : cpool.emplace(*this, entity, std::forward<Args>(args)...);
    }

    /*! @copydoc get_or_emplace */
    template<typename Component, typename... Args>
    [[deprecated("use ::get_or_emplace instead")]] decltype(auto) get_or_assign(
      const entity_type entity, Args&&... args)
    {
    ...
    void clear()
    {
        ...
        else
        {
            (
              [this](auto&& cpool) {
                  cpool.remove(*this,
                               cpool.sparse_set<entity_type>::begin(),
                               cpool.sparse_set<entity_type>::end());
              }(assure<Component>()),
              ...);
        }
    }
    ...
    void each(Func func) const
    {
        ...
        else
        {
            for (auto pos = entities.size(); pos; --pos)
            {
                if (const auto entt = entities[pos - 1];
                    (to_integral(entt) & traits_type::entity_mask) == (pos - 1))
                {
                    func(entt);
                }
            }
    ...
    {
        ENTT_ASSERT(valid(entity));
        return std::none_of(pools.cbegin(), pools.cend(), [entity](auto&& pdata) {
    ...
    /*! @copydoc on_update */
    template<typename Component>
    [[deprecated("use registry::on_update instead")]] auto on_replace()
    {
    ...
    /*! @copydoc view */
    template<typename... Component, typename... Exclude>
    entt::basic_view<Entity, exclude_t<Exclude...>, Component...> view(
    ...
    entt::basic_group<Entity, exclude_t<Exclude...>, get_t<Get...>, Owned...> group(
      get_t<Get...>, exclude_t<Exclude...> = {})
    {
        ...
        using handler_type =
          group_handler<exclude_t<Exclude...>, get_t<std::decay_t<Get>...>, std::decay_t<Owned>...>;

        const auto cpools =
          std::forward_as_tuple(assure<std::decay_t<Owned>>()..., assure<std::decay_t<Get>>()...);
        constexpr auto size    = sizeof...(Owned) + sizeof...(Get) + sizeof...(Exclude);
        handler_type*  handler = nullptr;

        if (auto it =
              std::find_if(groups.cbegin(),
                           groups.cend(),
        ...
        if (!handler)
        {
            ...
            else
            {
                ENTT_ASSERT(std::all_of(groups.cbegin(), groups.cend(), [size](const auto& gdata) {
                    const auto overlapping =
                      (0u + ... + gdata.owned(type_info<std::decay_t<Owned>>::id()));
                    const auto sz = overlapping +
                                    (0u + ... + gdata.get(type_info<std::decay_t<Get>>::id())) +
                                    (0u + ... + gdata.exclude(type_info<Exclude>::id()));
                    return !overlapping || ((sz == size) || (sz == gdata.size));
                }));

                const auto next =
                  std::find_if_not(groups.cbegin(), groups.cend(), [size](const auto& gdata) {
                      return !(0u + ... + gdata.owned(type_info<std::decay_t<Owned>>::id())) ||
                             (size > (gdata.size));
                  });

                const auto prev = std::find_if(
                  std::make_reverse_iterator(next), groups.crend(), [](const auto& gdata) {
                      return (0u + ... + gdata.owned(type_info<std::decay_t<Owned>>::id()));
                  });
            ...
            (on_construct<std::decay_t<Owned>>()
               ...
               .template connect<&handler_type::template maybe_valid_if<std::decay_t<Owned>>>(
                 *handler),
             ...);
            (on_construct<std::decay_t<Get>>()
               .before(maybe_valid_if)
               .template connect<&handler_type::template maybe_valid_if<std::decay_t<Get>>>(
                 *handler),
             ...);
            (on_destroy<Exclude>()
               .before(maybe_valid_if)
            ...
            (on_destroy<std::decay_t<Owned>>()
               .before(discard_if)
               .template connect<&handler_type::discard_if>(*handler),
             ...);
            (on_destroy<std::decay_t<Get>>()
               .before(discard_if)
               .template connect<&handler_type::discard_if>(*handler),
             ...);
            ...
            {
                // we cannot iterate backwards because we want to leave behind valid entities in
                // case of owned types
                for (auto *first = std::get<0>(cpools).data(),
                     ...
                     ++first)
                {
                    handler->template maybe_valid_if<
                      std::tuple_element_t<0, std::tuple<std::decay_t<Owned>...>>>(*this, *first);
                }
            }
        }
      ...
      get_t<Get...>, exclude_t<Exclude...> = {}) const
    {
        ...
        return const_cast<basic_registry*>(this)->group<Owned...>(entt::get<Get...>,
                                                                  exclude<Exclude...>);
    }
    ...
    entt::basic_group<Entity, exclude_t<Exclude...>, get_t<>, Owned...> group(
      exclude_t<Exclude...> = {})
    {
    ...
    entt::basic_runtime_view<Entity> runtime_view(It first, It last) const
    {
        ...
        std::transform(first, last, selected.begin(), [this](const auto ctype) {
            const auto it = std::find_if(pools.cbegin(), pools.cend(), [ctype](auto&& pdata) {
                return pdata.pool && pdata.type_id == ctype;
            });
     ...
     */
    [[deprecated(
      "basic_snapshot has now a constructor that accepts a reference to a registry")]] entt::
     ...
     */
    [[deprecated(
      "basic_snapshot_loader has now a constructor that accepts a reference to a "
      "registry")]] basic_snapshot_loader<Entity>
    loader()
    {
        return {*this};
    }
    ...
    {
        unset<Type>();
        vars.push_back(variable_data {type_info<Type>::id(),
    ...
    void unset()
    {
        vars.erase(std::remove_if(vars.begin(),
                                  vars.end(),
                                  [](auto&& var) { return var.type_id == type_info<Type>::id(); }),
                   vars.end());
    }
    ...
    {
        auto it = std::find_if(vars.cbegin(), vars.cend(), [](auto&& var) {
...
struct basic_actor
{
     ...
     */
    basic_actor(basic_actor&& other) ENTT_NOEXCEPT : entt {other.entt}, reg {other.reg}
    {
        other.entt = null;
    }
     ...
     * @param ref An instance of the registry class.
     */
    explicit basic_actor(entity_type entity, registry_type& ref) ENTT_NOEXCEPT : entt {entity},
                                                                                 reg {&ref}
    {
...
struct as_view
{
    ...
    using registry_type =
      std::conditional_t<Const, const entt::basic_registry<Entity>, entt::basic_registry<Entity>>;
...
struct basic_collector<>
{
    ...
    /*! @copydoc update */
    template<typename AnyOf>
    [[deprecated("use ::update instead")]] static constexpr auto replace() ENTT_NOEXCEPT
    {
        return update<AnyOf>();
    }
};
...
struct basic_collector<matcher<type_list<Reject...>, type_list<Require...>, Rule...>, Other...>
{
    ...
    {
        using extended_type =
          matcher<type_list<Reject..., NoneOf...>, type_list<Require..., AllOf...>, Rule...>;
...
class basic_observer
{
    ...
    struct matcher_handler<matcher<type_list<Reject...>, type_list<Require...>, AnyOf>>
    {
        ...
        static void maybe_valid_if(basic_observer&               obs,
        ...
        template<std::size_t Index>
        static void discard_if(basic_observer& obs,
    ...
    struct matcher_handler<
      matcher<type_list<Reject...>, type_list<Require...>, type_list<NoneOf...>, AllOf...>>
    {
        template<std::size_t Index>
        static void maybe_valid_if(basic_observer&               obs,
                                   const basic_registry<Entity>& reg,
                                   const Entity                  entt)
        {
            if (reg.template has<AllOf..., Require...>(entt) &&
                !reg.template any<NoneOf..., Reject...>(entt))
            {
                if (auto* comp = obs.view.try_get(entt); !comp)
                {
                    obs.view.emplace(entt);
...
public:
    /*! @brief Default constructor. */
    meta_any() ENTT_NOEXCEPT : storage {},
                               instance {},
                               node {},
                               destroy_fn {},
                               copy_fn {},
    ...
    template<typename Type,
             typename = std::enable_if_t<
               !std::is_same_v<std::remove_cv_t<std::remove_reference_t<Type>>, meta_any>>>
    meta_any(Type&& value)
    : meta_any {std::in_place_type<std::remove_cv_t<std::remove_reference_t<Type>>>,
    ...
    {
        node = other.node;
        (other.copy_fn ? other.copy_fn : [](meta_any& to, const meta_any& from) {
            to.instance = from.instance;
        })(*this, other);
     ...
     * @param other The instance to move from.
     */
    meta_any(meta_any&& other) noexcept : meta_any {} { swap(*this, other); }
    ...
    const Type* try_cast() const
    {
        ...
        if (const auto type_id = internal::meta_info<Type>::resolve()->type_id;
    ...
    bool operator==(const meta_any& other) const
    {
        return (!node && !other.node) ||
               (node && other.node && node->type_id == other.node->type_id &&
                node->compare(instance, other.instance));
    }
...
struct meta_ctor
{
    ...
    meta_prop prop(meta_any key) const
    {
        return internal::find_if(
          [key = std::move(key)](const auto* curr) { return curr->key() == key; }, node->prop);
    }
...
struct meta_data
{
    ...
    /*! @copydoc id */
    [[deprecated("use ::id instead")]] id_type alias() const ENTT_NOEXCEPT { return id(); }
...
struct meta_func
{
    ...
    meta_any invoke(meta_handle instance, Args&&... args) const
    {
        ...
        return sizeof...(Args) == size()
                 ? node->invoke(arguments[0], &arguments[sizeof...(Args) != 0])
                 : meta_any {};
    }
              ...
              return candidate->size == sizeof...(Args) && ([](auto* from, auto* to) {
                         return (from->type_id == to->type_id) ||
                                internal::find_if<&internal::meta_type_node::base>(
                                  [to](const auto* curr) {
                                      return curr->type()->type_id == to->type_id;
                                  },
                                  from) ||
...
public:
    ...
    {
        return internal::find_if<&internal::meta_type_node::data>(
    ...
    {
        return internal::find_if<&internal::meta_type_node::func>(
          [id](const auto* curr) { return curr->id == id; }, node);
    }
...
/*! @copydoc as_ref */
[[deprecated("use as_ref instead")]] inline constexpr as_ref_t as_alias;
...
struct meta_function_helper<Ret(Args...)>
{
    ...
    {
        return std::array<meta_type_node*, sizeof...(Args)> {
          {meta_info<Args>::resolve()...}}[index];
    }
};
...
template<typename Ret, typename... Args, typename Class>
constexpr meta_function_helper<Ret(Args...) const> to_meta_function_helper(Ret (Class::*)(Args...)
                                                                             const);
...
meta_any construct(meta_any* const args, std::index_sequence<Indexes...>)
{
    ...
    {
        any = Type {(std::get<Indexes>(direct) ? *std::get<Indexes>(direct)
                                               : (args + Indexes)->cast<Args>())...};
    }
...
bool setter([[maybe_unused]] meta_any instance,
            [[maybe_unused]] meta_any index,
            [[maybe_unused]] meta_any value)
{
    ...
    if constexpr (!Const)
    {
        if constexpr (std::is_function_v<
                        std::remove_reference_t<std::remove_pointer_t<decltype(Data)>>> ||
                      std::is_member_function_pointer_v<decltype(Data)>)
        {
            using helper_type = meta_function_helper_t<decltype(Data)>;
            using data_type =
              std::tuple_element_t<!std::is_member_function_pointer_v<decltype(Data)>,
        ...
        {
            using data_type =
              std::remove_cv_t<std::remove_reference_t<decltype(std::declval<Type>().*Data)>>;
            ...
            if constexpr (std::is_array_v<data_type>)
            {
                ...
                {
                    std::invoke(Data, clazz)[*idx] =
                      direct ? *direct : value.cast<underlying_type>();
                    accepted = true;
                }
            }
            else
...
meta_any invoke([[maybe_unused]] meta_any instance, meta_any* args, std::index_sequence<Indexes...>)
{
    ...
    [[maybe_unused]] const auto direct = std::make_tuple(
      [](meta_any* any, auto* value) {
        ...
        (args + Indexes)
          ->try_cast<std::tuple_element_t<Indexes, typename helper_type::args_type>>())...);

    if constexpr (std::is_function_v<
                    std::remove_reference_t<std::remove_pointer_t<decltype(Candidate)>>>)
    {
        return (std::get<Indexes>(direct) && ...) ? dispatch(std::get<Indexes>(direct)...)
                                                  : meta_any {};
    }
    else
    {
        auto* const clazz = instance.try_cast<Type>();
        return (clazz && (std::get<Indexes>(direct) && ...))
                 ? dispatch(clazz, std::get<Indexes>(direct)...)
                 : meta_any {};
    }
}
...
class meta_factory<Type, Spec...> : public meta_factory<Type>
{
    ...
    {
        unroll<Step>(
          choice<3>, std::move(std::get<Index>(property))..., std::forward<Other>(other)...);
    }

    template<std::size_t Step = 0, typename... Property, typename... Other>
    void unroll(choice_t<3>, std::tuple<Property...> property, Other&&... other)
    {
        unpack<Step>(std::index_sequence_for<Property...> {},
                     std::move(property),
    ...
    template<std::size_t Step = 0, typename Property, typename... Other>
    std::enable_if_t<!std::is_invocable_v<Property>> unroll(choice_t<1>,
                                                            Property&& property,
                                                            Other&&... other)
    {
    ...
    void assign(Key&& key, Value&&... value)
    {
        static const auto property {
          std::make_tuple(std::forward<Key>(key), std::forward<Value>(value)...)};

        static internal::meta_prop_node node {nullptr,
                                              []() -> meta_any { return std::get<0>(property); },
                                              []() -> meta_any {
...
public:
    ...
    /*! @copydoc type */
    [[deprecated("use ::type instead")]] auto alias(const id_type id) ENTT_NOEXCEPT
    {
        return type(id);
    }
    ...
    auto base() ENTT_NOEXCEPT
    {
        ...
        static internal::meta_base_node node {type,
                                              nullptr,
    ...
    auto conv() ENTT_NOEXCEPT
    {
        ...
        static internal::meta_conv_node node {
          type, nullptr, &internal::meta_info<To>::resolve, [](const void* instance) -> meta_any {
    ...
    auto conv() ENTT_NOEXCEPT
    {
        ...
        static internal::meta_conv_node node {type,
                                              nullptr,
    ...
    auto ctor() ENTT_NOEXCEPT
    {
        ...
        static internal::meta_ctor_node node {type,
                                              nullptr,
                                              nullptr,
                                              helper_type::index_sequence.size(),
                                              &helper_type::arg,
                                              [](meta_any* const any) {
                                                  return internal::invoke<Type, Func, Policy>(
    ...
    auto func(const id_type id) ENTT_NOEXCEPT
    {
        ...
        static internal::meta_func_node node {
          ...
          &internal::meta_info<std::conditional_t<std::is_same_v<Policy, as_void_t>,
                                                  void,
                                                  typename helper_type::return_type>>::resolve,
          &helper_type::arg,
          [](meta_any instance, meta_any* args) {
              return internal::invoke<Type, Candidate, Policy>(
                std::move(instance), args, helper_type::index_sequence);
          }};
        ...
        return meta_factory<Type, std::integral_constant<decltype(Candidate), Candidate>> {
          &node.prop};
    }
...
/*! @copydoc resolve_id */
[[deprecated("use entt::resolve_id instead")]] inline meta_type resolve(const id_type id)
  ENTT_NOEXCEPT
{
    ...
    struct continuation
    {
        ...
        continuation then(Args&&... args)
        {
            ...
            handler->next.reset(new process_handler {
              std::move(proc), &scheduler::update<Proc>, &scheduler::abort<Proc>, nullptr});
    ...
    void update(const Delta delta, void* data = nullptr)
    {
        ...
        if (clean)
        {
            handlers.erase(std::remove_if(handlers.begin(),
                                          handlers.end(),
                                          [](auto& handler) { return !handler.instance; }),
                           handlers.end());
        }
    }
...
class dispatcher
{
    ...
    pool_handler<Event>& assure()
    {
        ...
        {
            auto it = std::find_if(
              pools.begin(), pools.end(), [id = type_info<Event>::id()](const auto& cpool) {
                  return id == cpool->type_id();
              });
            return static_cast<pool_handler<Event>&>(
              it == pools.cend() ? *pools.emplace_back(new pool_handler<Event> {}) : **it);
        }
    }
...
public:
    ...
    {
        return std::all_of(
          pools.cbegin(), pools.cend(), [](auto&& cpool) { return !cpool || cpool->empty(); });
    }
