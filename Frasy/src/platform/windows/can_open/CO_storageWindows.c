...
static ODR_t storeWindows(CO_storage_entry_t* entry, CO_CANmodule_t* CANmodule)
{
    ...
    if (ret == ODR_OK) {
        ...
        if (fp == NULL) { ret = ODR_HW; }
            ...
            fclose(fp);
            if (cnt != (entry->len + sizeof(crc_store))) { ret = ODR_HW; }
        }
    }
    ...
    if (ret == ODR_OK) {
        ...
        if (rename(filename_tmp, entry->filename) != 0) { ret = ODR_HW; }
    }
 ...
 * For more information see file CO_storage.h, CO_storage_entry_t.
 */
static ODR_t restoreWindows(CO_storage_entry_t* entry, CO_CANmodule_t* CANmodule)
{
    (void)CANmodule;
    ODR_t ret = ODR_OK;
    ...
    if (filename_old == NULL) { ret = ODR_OUT_OF_MEM; }
    else {
        strcpy(filename_old, entry->filename);
        strcat(filename_old, ".old");
        rename(entry->filename, filename_old);
                                        ...
                                        uint32_t*           storageInitError)
{
    ...
    CO_ReturnError_t ret = CO_storage_init(storage,
                                           CANmodule,
...
uint32_t CO_storageWindows_auto_process(CO_storage_t* storage, bool_t closeFiles)
{
    ...
    for (uint8_t i = 0; i < storage->entriesCount; i++) {
        ...
        if ((entry->attr & CO_storage_auto) == 0 || entry->fp == NULL) { continue; }
        ...
        if (crc != entry->crc) {
            ...
            fflush(entry->fp);
            if (cnt == (entry->len + sizeof(crc))) { entry->crc = crc; }
            else {
                /* error with save */
                uint32_t errorBit = entry->subIndexOD;
                if (errorBit > 31) { errorBit = 31; }
