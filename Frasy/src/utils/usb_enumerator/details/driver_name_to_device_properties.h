...
inline std::optional<DevicePnpStrings> DriverNameToDeviceProperties(const std::string& driverName)
{
    ...
    status = SetupDiGetDeviceInstanceIdA(deviceInfo, &deviceInfoData, (PSTR)devicePnpStrings.deviceId.data(), requiredLen, &requiredLen);
    if (status == FALSE) {
        FRASY_USB_OOPS();
        return std::nullopt;
    }
    ...
    if (maybeDesc.has_value()) {
        devicePnpStrings.deviceDesc = maybeDesc.value();
    } else {
        FRASY_USB_OOPS();
        return std::nullopt;
    }
