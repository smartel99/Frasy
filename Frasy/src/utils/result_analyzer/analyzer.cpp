  ...
  const std::string& path, const std::vector<std::array<char, 32>>& serialNumbers)
{
    ...
    auto numbers = [&serialNumbers]()
    {
        ...
        for (auto&& number : serialNumbers) { numbers.emplace_back(number.data()); }
...
std::vector<std::string> LoadAllMatchingFiles(const std::vector<std::array<char, 32>>& serialNumbers)
{
    auto r   = LoadMatchingFiles("logs/fail", serialNumbers);
    auto tmp = LoadMatchingFiles("logs/pass", serialNumbers);
...
ResultAnalysisResults ResultAnalyzer::Analyze()
{
    m_results = {};
    // Load all files that match the options from the pass and fail directory
    auto logs = LoadAllMatchingFiles(m_options.SerialNumbers);
    ToAnalyze = logs.size();
...
void ResultAnalyzer::AnalyzeFile(const std::string& path)
{
                                  ...
                                  { return location.find(std::string_view(u.data())) != std::string::npos; }))
    {
        ...
        for (auto [name, sequence] : sequences.items())
        {
            ...
            else { BR_LOG_DEBUG("Analyzer", "Ignored sequence '{}'...", name); }
        }
    }
}
...
void ResultAnalyzer::AnalyzeSequence(const nlohmann::json& sequence, ResultAnalysisResults::Sequence& results)
{
    ...
    for (auto [name, test] : tests.items())
    {
        ...
        else { BR_LOG_DEBUG("Analyzer", "Ignored test '{}'...", name); }
    }
}
