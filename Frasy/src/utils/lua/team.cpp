...
void Team::InitializeState(sol::state &other, std::size_t uut, std::size_t position, bool is_leader) {
    ...
    other["Team"]["__sync"] = [&](int status) {
        ...
        if (std::any_of(m_syncStates.begin(),
                        m_syncStates.end(),
                        [](SyncState state) { return state == SyncState::fail; })) {
...
template<typename T> void Team::Serialize(const T &t) {
...
template<typename T> T Team::Deserialize(std::size_t &cur) {
    std::size_t i = cur;
    if constexpr (std::is_same_v<T, std::string>) {
...
std::optional<sol::object> Team::_Load(sol::state &lua, std::size_t &cur) {
    ...
    if (tp == sol::type::number) {
        return sol::make_object(lua, Deserialize<double>(cur));
    }
    else if (tp == sol::type::boolean) {
        return sol::make_object(lua, Deserialize<bool>(cur));
    }
    else if (tp == sol::type::string) {
        return sol::make_object(lua, Deserialize<std::string>(cur));
    }
    else if (tp == sol::type::table) {
