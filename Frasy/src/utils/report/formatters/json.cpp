...
namespace Frasy::Report::Formatter {
    ...
    void Json::reportTestResult(const std::string& name) {
        ...
        auto obj = nlohmann::ordered_json::object();
        m_section = &obj;
        reportSectionBaseResult(
            m_result["sequences"][m_sequenceName]["tests"].get_or(m_emptyTable)[name].get_or(m_emptyTable));
        m_section = nullptr;
    ...
    void Json::toFile(const std::string& filename) {
        ...
        ofs << m_object.dump(4, ' ', true, nlohmann::detail::error_handler_t::ignore) << std::endl;
    }
