...
{
    lua.script(R"(
        Expectation:New(100, "val", { onErrorExtra = { debug = "out of range" } }):ToBeInRange(0, 50)
    )");
...
{
    lua.script(R"(
        Expectation:New(5, "val", { onErrorExtra = { debug = "should not appear" } }):ToBeInRange(0, 10)
    )");
...
{
    lua.script(R"(
        Expectation:New(false, "flag", { onErrorExtra = { a = 1, b = "two" } }):ToBeTrue()
    )");
